...
@Resource(name = RestConstants.VERSION_1 + PharmacyRest.PHARMA_NAMESPACE + "/itemsLine", supportedClass = ItemsLine.class, supportedOpenmrsVersions = { "2.*.*" })
public class ItemsLineRequestResource extends DelegatingCrudResource<ItemsLine> {
	...
	private ItemsLine constructAgent(String uuid, SimpleObject properties) {
		...
		if (uuid != null) {
			...
			if (properties.get("location") != null) {
				itemsLine.setLocation(location);
			}
