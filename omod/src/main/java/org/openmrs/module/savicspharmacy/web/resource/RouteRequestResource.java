...
@Resource(name = RestConstants.VERSION_1 + PharmacyRest.PHARMA_NAMESPACE + "/route", supportedClass = Route.class, supportedOpenmrsVersions = { "2.*.*" })
public class RouteRequestResource extends DataDelegatingCrudResource<Route> {
	...
	protected PageableResult doGetAll(RequestContext context) throws ResponseException {
		List<Route> districtList = Context.getService(PharmacyService.class).getAll(Route.class, context.getLimit(),
		    context.getStartIndex());
		System.out.println(districtList.toString());
		return new AlreadyPaged<Route>(context, districtList, false);
	}
	
	@Override
	protected PageableResult doSearch(RequestContext context) {
		String value = context.getParameter("name");
		List<Route> districtList = Context.getService(PharmacyService.class).doSearch(Route.class, "name", value,
	...
	public Object create(SimpleObject propertiesToCreate, RequestContext context) throws ResponseException {
		...
		System.out.println();
		Route route = this.constructDistrict(null, propertiesToCreate);
	...
	public Object update(String uuid, SimpleObject propertiesToUpdate, RequestContext context) throws ResponseException {
		Route route = this.constructDistrict(uuid, propertiesToUpdate);
		Context.getService(PharmacyService.class).upsert(route);
		return ConversionUtil.convertToRepresentation(route, context.getRepresentation());
	}
	...
	private Route constructDistrict(String uuid, SimpleObject properties) {
		Route route;
