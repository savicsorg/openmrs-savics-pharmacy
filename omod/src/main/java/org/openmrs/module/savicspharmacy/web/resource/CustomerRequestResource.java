...
@Resource(name = RestConstants.VERSION_1 + PharmacyRest.PHARMA_NAMESPACE + "/customer", supportedClass = Customer.class, supportedOpenmrsVersions = { "2.*.*" })
public class CustomerRequestResource extends DataDelegatingCrudResource<Customer> {
	...
	public DelegatingResourceDescription getRepresentationDescription(Representation rep) {
		if (rep instanceof DefaultRepresentation) {
			...
			description.addProperty("customerTypeId");
	...
	private Customer constructCustomer(String uuid, SimpleObject properties) {
		...
		if (properties.get("customerTypeId") != null) {
			Integer customerTypeId = properties.get("customerTypeId");
			customerType = (CustomerType) Context.getService(PharmacyService.class).getEntityByid(CustomerType.class,
			    "regionId", customerTypeId);
		}
		if (uuid != null) {
