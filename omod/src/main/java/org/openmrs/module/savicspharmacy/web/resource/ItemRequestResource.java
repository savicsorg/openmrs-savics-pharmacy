...
@Resource(name = RestConstants.VERSION_1 + PharmacyRest.PHARMA_NAMESPACE + "/item", supportedClass = Item.class, supportedOpenmrsVersions = { "2.*.*" })
public class ItemRequestResource extends DataDelegatingCrudResource<Item> {
	...
	public DelegatingResourceDescription getRepresentationDescription(Representation rep) {
		if (rep instanceof DefaultRepresentation) {
			...
			description.addProperty("unitId");
			description.addProperty("routeId");
	...
	private Item constructItem(String uuid, SimpleObject properties) {
		Item item;
		Unit unit = null;
		if (properties.get("unitId") != null) {
			Integer unitId = properties.get("unitId");
			unit = (Unit) Context.getService(PharmacyService.class).getEntityByid(Unit.class, "itemTypeId", unitId);
		}
		Route route = null;
		if (properties.get("routeId") != null) {
			Integer routeId = properties.get("routeId");
			route = (Route) Context.getService(PharmacyService.class).getEntityByid(Route.class, "routeId", routeId);
		}
		if (uuid != null) {
